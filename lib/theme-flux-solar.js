...
const LOCATION_FALLBACK_URL = 'https://maps.googleapis.com/maps/api/browserlocation/json?browser=chromium&sensor=true';
...
export default {
  ...
  changeTheme() {
    navigator.geolocation.getCurrentPosition(
      geo => this.changeThemeGeo(geo),
      () => fetch(LOCATION_FALLBACK_URL)
        .then(r => r.json())
        .then(({ location: { lat, lng } }) => ({ coords: { latitude: lat, longitude: lng } }))
        .then(geo => this.changeThemeGeo(geo))
    );
  },

  changeThemeGeo({ coords: { latitude, longitude } }) {
