...
export default {
  ...
  changeTheme() {
    ...
    if (apiKey) {
      fetch(`${API_URL}?key=${apiKey}`, { method: 'POST' })
        ...
        .then(() => {
          if (!localStorage.getItem(`${PACKAGE_NAME}.first`)) {
            localStorage.setItem(`${PACKAGE_NAME}.first`, true);
        ...
        .catch(() => {
          localStorage.removeItem(`${PACKAGE_NAME}.first`);
  ...
  \`theme-flux-solar\` could not retrive geo location. Maybe the API key is invalid?
    ...
    } else if (!localStorage.getItem(`${PACKAGE_NAME}.welcome`)) {
      localStorage.setItem(`${PACKAGE_NAME}.welcome`, true);
