...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.config.observe(PACKAGE_NAME, () => {
      this.wasDay = null; // force theme update
      clearTimeout(id);
      id = setTimeout(this.changeTheme, 1000);
    }));
  ...
  changeTheme() {
    ...
    if (apiKey) {
      fetch(`${API_URL}?key=${apiKey}`, { method: 'POST' })
        .then(x => x.json())
        .then(this.changeThemeGeo)
        .then(() => {
          if (!atom.config.get(`${PACKAGE_NAME}.first`)) {
            atom.config.set(`${PACKAGE_NAME}.first`, true);
        ...
        .catch(() => {
          atom.config.set(`${PACKAGE_NAME}.first`, false);
          atom.notifications.addError('Could not retrive geolocation', {
            description: String.prototype.trim.call(`
  \`theme-flux-solar\` could not retrive geo location. Maybe the API key is invalid?
    ...
    } else if (!this.infoNotif) {
      this.infoNotif = atom.notifications.addInfo('API key required', {
        description: API_DESCRIPTION,
        buttons: [{
          text: 'Set API Key',
          onDidClick: () => atom.workspace.open(`atom://config/packages/${PACKAGE_NAME}`),
